...
class Cat_Puestos extends CI_Controller{
        ...
        if ($total <= 0) {
          ...
          foreach($rows as $r){
            if($r[0] != ''){
              ...
              $hayId = $this->cat_puestos_model->check($puesto);
              if($hayId == 0){
                $data = array(
                  'creacion' => $date,
                  'edicion' => $date,
                ...
                $this->cat_puestos_model->registrar($data);
              }
            }
          }
          $msj = array(
  ...
  function getPositionByName(){
    ...
    $existPosition = $this->cat_puestos_model->getPositionByName($this->input->post('nombre')) ?? 'Na';
    ...
    if($existPosition == NULL){
      ...
      $id = $this->cat_puestos_model->addPositionWithIdReturned($position);
    }else{
      $id = $existPosition->id;
    }
    echo $id;
  ...
  function getAllPositions(){
    $res = $this->cat_puestos_model->getAllPositions();
    if($res != NULL){
  ...
  function getAll(){
    $res = $this->cat_puestos_model->getAll();
    if($res != null){
      echo json_encode($res);
    }
    else{

...
class Reclutamiento_model extends CI_Model{
    ...
    try {
		...
		function editarDatosAspiranteBolsa($datos_bolsa, $id_bolsa, $datos_aspirante, $id_aspirante) {
			// Iniciar transacción
			$this->db->trans_start();
	...
	function getDetailsOrderById($id){
      ...
      ->join('requisicion as R','R.id = H.id_requisicion')
		...
		function getAspirantesRequisiciones($id_usuario, $condicion){
			...
			$this->db
			->select("A.*, CONCAT(BT.nombre,' ',BT.paterno,' ',BT.materno) as aspirante, CONCAT(GENCL.nombre,' ',GENCL.paterno) as usuario, BT.domicilio, BT.medio_contacto, BT.area_interes,  BT.telefono,  R.id as id_req, CL.nombre as nombre_cliente, CL.clave, CL.id as id_cliente, R.puesto as req_puesto, H.id as idHistorial, R.numero_vacantes")
			->from('requisicion_aspirante as A')
   ...
   function getCandidatosByRequisicion($id_requisicion){
    ...
    function getBolsaTrabajoByName($nombre, $paterno, $materno, $id_portal){
			$id_portal = $this->session->userdata('idPortal');
    ...
    function getBolsaTrabajoByPhone($telefono, $id_portal = null ){
			$id_portal = $this->session->userdata('idPortal');

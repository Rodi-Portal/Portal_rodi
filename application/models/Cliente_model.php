...
class Cliente_model extends CI_Model
{
    ...
    {
        $idCliente     = $this->session->userdata('idcliente');
        $column_search = ['c.nombre', 'c.paterno', 'c.materno', 'S.proyecto', 'c.fecha_alta'];
        $order         = ['c.nombre' => 'asc'];
        $this->db
    ...
    public function get_requisicion_details($id_requisicion)
    {
...
/*
   ...
   die();*/
    ...
    public function get_current_procedures2()
    {
        $id_usuario = $this->session->userdata('id');
        $id_cliente = $this->session->userdata('idcliente');

        $this->db
            ->select("R.*,RIN.*, RIN.creacion as creacionRin, RIN.edicion as edicionRin, RIN.id as idRin,  R.id as idReq, CL.*, R.status as statusReq")
            ->from('requisicion as R')
            ->join('cliente as CL', 'CL.id = R.id_cliente')
            ->join('requisicion_intake as RIN', 'R.id_intake = RIN.id', 'left')
            ->join('datos_generales as DATCL', 'DATCL.id = CL.id_datos_generales', 'left')
    ...
    public function get_current_procedures3()
    {
        $id_usuario = $this->session->userdata('id');
        $id_cliente = $this->session->userdata('idcliente');
    ...
    */
    public function verificarMismoUsuarioCliente($correo, $id_usuario_cliente)
    {
        $this->db
            ->select("id")
    ...
    public function getEscuelasTotal($id_cliente)
    {
        $this->db
            ->select("*")
            ->from("alumno")
    ...
    public function getEscuelas($id_cliente)
    {
        $this->db
    ...
    public function getIntelaTotal($id_cliente)
    {
        $this->db
            ->select("*")
            ->from("candidato")
    ...
    public function getIntela($id_cliente)
    {
        $this->db
    ...
    public function getClientesEspanol()
    {
        $this->db
            ->select("*")
            ->from('cliente')
    ...
    public function getSubclientes($id_cliente)
    {
        $this->db
    ...
    public function checkIngles($id_cliente)
    {
        $this->db
            ->select("ingles")
            ->from('cliente')
    ...
    public function getCandidatosSubclientesTotal($id_cliente)
    {
        $this->db
            ->select("c.id")
            ->from("candidato as c")
    ...
    public function getCandidatosSubclientes($id_cliente)
    {
        $this->db
    ...
    public function getCandidatosSubclienteTotal($id_cliente, $id_subcliente)
    {
        $this->db
            ->select("c.id")
            ->from("candidato as c")
    ...
    public function getCandidatosSubcliente($id_cliente, $id_subcliente)
    {
        $this->db
            ->select("c.*, CONCAT(c.nombre,' ',c.paterno,' ',c.materno) as nombreCompleto, est.id as idEstudios,est.primaria_periodo, est.primaria_escuela, est.primaria_ciudad, est.primaria_certificado, est.primaria_validada, est.secundaria_periodo, est.secundaria_escuela, est.secundaria_ciudad, est.secundaria_certificado, est.secundaria_validada, est.preparatoria_periodo, est.preparatoria_escuela, est.preparatoria_ciudad, est.preparatoria_certificado, est.preparatoria_validada, est.licenciatura_periodo, est.licenciatura_escuela, est.licenciatura_ciudad, est.licenciatura_certificado, est.licenciatura_validada, est.otros_certificados, est.comentarios, est.carrera_inactivo, sub.empresa as subcliente, bgc.creacion as fecha_bgc")
            ->from('candidato as c')
    ...
    public function getCandidatosCliente($id_cliente, $privacidad_candidato)
    {
    ...
    public function getTotalCandidatosCliente($id_cliente)
    {
        $this->db
            ->select(" c.id")
            ->from('candidato as c')
    ...
    public function getPaqueteSubclienteProyecto($id_cliente, $id_proyecto)
    {
        $this->db
            ->select('paq.*')
    ...
    public function getTiposStudies()
    {
        $this->db
            ->select('*')
            ->from('tipo_studies');

    ...
    <div class="card shadow">
      <div class="card-header bg-gradient-primary text-white">
        <h2 class="mb-0" style="color:white!important;">
          <?php echo t('reportes_rot_title','Generar reporte de Exempleados'); ?>
        </h2>
      </div>

      <div class="card-body">
        <form method="post" action="<?php echo site_url('Reporte/exportar_excel_rotacion') ?>">
    ...
    if (sucursal_id) {
      $.ajax({
        ...
        success: function(data) {
          $('#puesto').html(
            '<option value=""><?php echo t('reportes_ex_select_puesto','-- Selecciona puesto --'); ?></option>'
            );
          $('#departamento').html(
            '<option value=""><?php echo t('reportes_ex_select_departamento','-- Selecciona departamento --'); ?></option>'
            );

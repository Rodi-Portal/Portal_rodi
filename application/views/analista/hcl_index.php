...
<div class="container-fluid">
	...
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
		<h1 class="h3 mb-0 text-gray-800">Cliente: <small> <?php echo $cliente->nombre ?> </small></h1><br>
							...
							<div class="col-6">
								<form id="candidato_reflaboral' . $i . '" class="formCandidato">
									...
									<div class="row">
										...
										<div class="col-md-9">
											<input type="text" class="form-control fecha_laboral es_reflab" name="reflab' . $i . '_entrada_ingles" id="reflab' . $i . '_entrada_ingles">
									<br>
										</div>
									</div>
									<div class="row">
										...
										<div class="col-md-9">
											<input type="text" class="form-control fecha_laboral es_reflab" name="reflab' . $i . '_salida_ingles" id="reflab' . $i . '_salida_ingles">
									<br>
										</div>
									</div>
									<div class="row">
							...
							<div class="col-6">
  							<form id="analista_reflaboral' . $i . '" class="formAnalista">
								...
								<div class="row">
									...
									<div class="col-md-9">
										<input type="text" class="form-control fecha_laboral es_verlab" name="an_reflab' . $i . '_entrada" id="an_reflab' . $i . '_entrada">
										<br>
									</div>
								</div>
								<div class="row">
									...
									<div class="col-md-9">
										<input type="text" class="form-control fecha_laboral es_verlab" name="an_reflab' . $i . '_salida" id="an_reflab' . $i . '_salida">
										<br>
									</div>
								</div>
								<div class="row">
...
<script>
	...
	var extras = [];
	var url = '<?php echo base_url('Client/getCandidatos?id_cliente='); ?>' + id_cliente;
	$(document).ready(function(){
		$('#fecha_nacimiento_registro, #fecha_nacimiento_nuevo, #fecha_facis').inputmask('mm/dd/yyyy', {
		...
		$('#tabla').DataTable({
			...
			"columns":[ 
				...
				{ title: 'Proyecto', data: 'proyecto', bSortable: false,
					mRender: function(data, type, full){
						if(data != null){
							var pais = (full.pais != null)? full.pais:'';
							return data+' '+pais;
						}
						else{
							//var pais = (full.pais != null)? full.pais:'-';
				...
				{ title: 'Fechas de Registro', data: 'fecha_alta', bSortable: false, "width": "15%",
					mRender: function(data, type, full){
				...
				{ title: 'Fechas de Proceso', data: 'fecha_contestado', bSortable: false, "width": "15%",
					mRender: function(data, type, full){
						if(full.cancelado == 0){
							if(full.socioeconomico == 1){
					...
					mRender: function(data, type, full) {
						if(full.cancelado == 0){
							...
							if(full.socioeconomico == 1){
								...
								var documentos_requeridos = '<a href="javascript:void(0)" id="documentos_requeridos" class="fa-tooltip icono_datatable"><i class="fas fa-folder-open"></i></a>';
								if(full.tipo_formulario > 0){
									return '<a href="javascript:void(0)" id="msj_avances" class="fa-tooltip icono_datatable"><i class="fas fa-comment-dots"></i></a> <a href="javascript:void(0)" id="generar" class="fa-tooltip icono_datatable"><i class="fas fa-key"></i></a> <a href="javascript:void(0)" id="editar_pruebas" class="fa-tooltip icono_datatable"><i class="fas fa-edit"></i></a> '+icono_datosContacto+' '+icono_fechaReal+' '+documentos_requeridos;
								}
								else{
									return '<a href="javascript:void(0)" id="msj_avances" class="fa-tooltip icono_datatable"><i class="fas fa-comment-dots"></i></a> <a href="javascript:void(0)" id="editar_pruebas" class="fa-tooltip icono_datatable"><i class="fas fa-edit"></i></a> '+icono_datosContacto+' '+icono_fechaReal+' '+documentos_requeridos;
								}
							}
							else{
          ...
          if(data.id_seccion_global_search == 102){
						$("#law_enforcement").prop('readonly', false);
						$("#regulatory").prop('readonly', false);
						$("#sanctions").prop('readonly', false);
						$("#other_bodies").prop('readonly', false);
				...
				$('a#datos_verificacion_docs', row).bind('click', () => {
                  ...
                  // if(aux[0] == 5){
									// 	$("#doc_curp").append("<a href='"+href+aux[1]+"' target='_blank' style='color: black;'>"+aux[1]+"</a><br><br>");
									// }
                  if(aux[0] == 6){
                  ...
                  if(aux[0] == 44){
					...
					$.ajax({
						url: '<?php echo base_url('Client/getVerificacionDocumentos'); ?>',
						method: 'POST',
						data: {'id_candidato':data.id},
						beforeSend: function(){
							$('.loader').css("display","block");
            ...
            success: function(res){
					...
					$.ajax({
						async: false,
						url: '<?php echo base_url('Client/getReferenciasLaborales'); ?>',
						method: 'POST',
						data: {
					...
					$.ajax({
						async: false,
						url: '<?php echo base_url('Client/getVerificacionesLaborales'); ?>',
						method: 'POST',
						data: {
							'id_candidato': data.id
						},
							...
							if (res != "") {
								...
								for (var i = 0; i < rows.length; i++) {
									if (rows[i] != "") {
										...
										$("#an_reflab" + dato[28] + "_entrada").val(dato[2]);
										$("#an_reflab" + dato[28] + "_salida").val(dato[3]);
				...
				$('a#datos_domicilios_a', row).bind('click', () => {
					...
					$.ajax({
						url: '<?php echo base_url('Client/getHistorialDomicilios'); ?>',
						type: 'post',
						data: {'id_candidato':data.id},
						beforeSend: function(){
								$('.loader').css("display","block");
					...
					$.ajax({
						url: '<?php echo base_url('Client/getVerificacionHistorialDomicilios'); ?>',
						type: 'post',
						data: {'id_candidato':data.id},
						success : function(res)
						{ 
				...
				$('a#datos_ref_profesionales', row).bind('click', () => {
					...
					$.ajax({
						url: '<?php echo base_url('Client/getReferenciasProfesionales'); ?>',
						type: 'post',
						data: {'id_candidato':data.id},
						beforeSend: function(){
							$('.loader').css("display","block");
				...
				$('a#datos_ref_personales', row).bind('click', () => {
					...
					$.ajax({
						url: '<?php echo base_url('Client/getReferenciasPersonales'); ?>',
						type: 'post',
						data: {'id_candidato':data.id},
						beforeSend: function(){
							$('.loader').css("display","block");
        ...
        $('a#datos_ref_academicas', row).bind('click', () => {
          ...
          let tieneMilitar = getDocumentacion(data.id);
					...
					//$('.estatus_finales').prop('disabled',false);
					$(".estatus_finales").val(3);
          ...
          //Ref profesionales
					var lista_docs = (data.cantidad_ref_profesionales > 0)? '<li>Verificación de referencias profesionales</li>':'';
					listado += lista_docs;
          ...
          // $("#check_mvr").val(3);
          // $("#check_credencial_academica").val(3);
          // $.ajax({
          ...
          //         }
          //         if(docs[i]['id_tipo_documento'] == 15){
					// 			    $("#check_servicio_militar").prop('disabled',false);
          //           $("#check_servicio_militar").val('')
          //         }
          //         // if(docs[i]['id_tipo_documento'] == 37){
					// 			  //   $("#check_licencia_manejo").prop('disabled',false);
          //         //   $("#check_licencia_manejo").val('')
          //         // }
          ...
          //           $("#check_mvr").val('')
          //         }
          //         if(docs[i]['id_tipo_documento'] == 48){
					// 			    $("#check_credencial_academica").prop('disabled',false);
          //           $("#check_credencial_academica").val('')
          //         }
          //       }
					// 		}
          //   }
          ...
          // });
            ...
            success: function(res){
                ...
                let ciudadania = (dato.ciudadania_check == -1)? 3 : dato.ciudadania_check;
					      ...
					      //$("#check_licencia_manejo").val(dato.licencia_manejo_check);
				...
				$("a#actualizar_checks", row).bind('click', () => {
					$("#idCandidato").val(data.id);
					$(".nombreCandidato").text(data.candidato);
					$('.campoChecklist').prop('disabled', true)
					//$('.campoChecklist').val('')

          let tieneMilitar = getDocumentacion(data.id);
					//Se inician como deshabilitados todos los campos
					$("#checklist_visita").val(3);

					//Verificacion de identidad o de documentos
					var identidad = (data.lleva_identidad == 1)? false:true;
					$("#checklist_identidad").prop('disabled',identidad);
					var valor_identidad = (data.lleva_identidad == 1)? '':3;
					$("#checklist_identidad").val(valor_identidad);

					//Laborales
					var laboral = (data.lleva_empleos == 1)? false:true;
					$("#checklist_laboral").prop('disabled',laboral);
					var valor_empleos = (data.lleva_empleos == 1)? '':3;
					$("#checklist_laboral").val(valor_empleos);
					
					//Estudios
					var estudios = (data.lleva_estudios == 1)? false:true;
					$("#checklist_estudios").prop('disabled',estudios);
					var valor_estudios = (data.lleva_estudios == 1)? '':3;
					$("#checklist_estudios").val(valor_estudios);

					// //Examen Medico
					// $("#checklist_medico").prop('disabled',false);
					// $("#checklist_medico").val('');

					//Globales
					var globales = (data.id_seccion_global_search != null)? false:true;
					$("#checklist_global").prop('disabled',globales);
					var valor_global = (data.id_seccion_global_search != null)? '':3;
					$("#checklist_global").val(valor_global);

					// //Domicilios
					// var domicilios = (data.lleva_domicilios == 1)? false:true;
					// $("#checklist_domicilio").prop('disabled',domicilios);
					// var valor_domicilios = (data.lleva_domicilios == 1)? '':3;
					// $("#checklist_domicilio").val(valor_domicilios);
					
					// //Credito
					// var credito = (data.lleva_credito == 1)? false:true;
					// $("#checklist_credito").prop('disabled',credito);
					// var valor_credito = (data.lleva_credito == 1)? '':3;
					// $("#checklist_credito").val(valor_credito);

					//Verificacion criminal
					var criminales = (data.lleva_criminal == 1)? false:true;
					$("#checklist_penales").prop('disabled',criminales);
					$("#checklist_ofac").prop('disabled',false);
					var valor_criminal = (data.lleva_criminal == 1)? '':3;
					$("#checklist_penales").val(valor_criminal);
					$("#checklist_ofac").val('');
            ...
            success: function(res){
                ...
                let nss = (dato.nss_check == -1)? 3 : dato.nss_check;
                let ciudadania = (dato.ciudadania_check == -1)? 3 : dato.ciudadania_check;
								$("#checklist_identidad").val(dato.identidad_check);
								$("#checklist_laboral").val(dato.empleo_check);
								$("#checklist_estudios").val(dato.estudios_check);
								//$("#checklist_visita").val(dato.visita_check);
								$("#checklist_penales").val(dato.penales_check);
								$("#checklist_ofac").val(dato.ofac_check);
								//$("#checklist_laboratorio").val(dato.laboratorio_check);
								//$("#checklist_medico").val(dato.medico_check);
								$("#checklist_global").val(dato.global_searches_check);
								//$("#checklist_domicilio").val(dato.domicilios_check);
								//$("#checklist_credito").val(dato.credito_check);
								//$("#checklist_professional_accreditation").val(dato.professional_accreditation_check);
								//$("#checklist_sex_offender").val(dato.sex_offender_check);
								//$("#checklist_ref_academica").val(dato.ref_academica_check);
								//$("#checklist_nss").val(nss);
								//$("#checklist_ciudadania").val(ciudadania);
								//$("#checklist_mvr").val(dato.mvr_check);
					      //$("#checklist_servicio_militar").val(dato.militar_check);
					      //$("#checklist_credencial_academica").val(dato.credencial_academica_check);
					      //$("#checklist_licencia_manejo").val(dato.licencia_manejo_check);
					      //$("#checklist_ref_profesional").val(dato.ref_profesional_check);
							}
							else{
		...
		$("#region").change(function(){
            ...
            //CURP
						...
						$('#curp_registro').append($('<option></option>').attr('value',0).attr("selected","selected").text('N/A'));
					}
				});
				if(region == 'International'){
					$('#pais_registro').prop('disabled', false);
					$('#pais_registro').val('');
					$('#pais_registro').find('option[value="México"]').remove()
					$('#mvr_registro').val(0);
				}	      	
				else{
					$('#pais_registro').prop('disabled', true);
					//$('#pais_registro').val('México');
					$('#pais_registro').append($('<option></option>').attr('value','México').attr("selected","selected").text('Mexico'));
				}
    ...
    function guardarRefAcademica(num) {
      ...
      $.ajax({
		...
		function verificarDocumentacion(id_candidato, pais){
			$.ajax({
				url: '<?php echo base_url('Client/getDocumentosRequeridos'); ?>',
				method: 'POST',
				data: {'id_candidato':id_candidato},
				beforeSend: function(){
					$('.loader').css("display","block");
            ...
            if(conjunto.includes('5')){ //CURP
							$("#curp_numero").prop('readonly', false);
						}
		...
		function estatusAvances() {
			var id_rol = <?php echo json_encode($this->session->userdata('id_rol')); ?>;
      ...
      datos.append('mvr', $('#mvr').val());
		...
		function actualizarTrabajoGobierno() {
			...
			$.ajax({
				url: '<?php echo base_url('Client/guardarTrabajoGobierno'); ?>',
				type: 'POST',
				data: datos,
				contentType: false,
				cache: false,
		...
		function actualizarReferenciaLaboral(num) {
			...
			$.ajax({
				url: '<?php echo base_url('Client/guardarReferenciaLaboral'); ?>',
				type: 'POST',
				data: datos,
				contentType: false,
				cache: false,
		...
		function verificarLaboral(num) {
			...
			$.ajax({
				url: '<?php echo base_url('Client/guardarVerificacionLaboral'); ?>',
				type: 'POST',
				data: datos,
				contentType: false,
				cache: false,
      ...
      datos.append('nss_numero', $("#nss_numero").val());
			...
			//datos.append('curp', $("#curp_numero").val());
			datos.append('comentarios', $("#doc_comentarios").val());
			datos.append('id_candidato', $("#idCandidato").val());
			
			$.ajax({
				url: '<?php echo base_url('Client/guardarDocumentacion'); ?>',
				type: 'POST',
				data: datos,
				contentType: false,
				cache: false,
		...
		function finalizarProceso(){
			...
			//datos.append('check_licencia_manejo', $("#check_licencia_manejo").val());
		...
		function actualizarChecks(){
			var datos = new FormData();
			datos.append('check_identidad', $("#checklist_identidad").val());
			datos.append('check_laboral', $("#checklist_laboral").val());
			datos.append('check_estudios', $("#checklist_estudios").val());
			datos.append('check_visita', $("#checklist_visita").val());
			datos.append('check_penales', $("#checklist_penales").val());
			datos.append('check_ofac', $("#checklist_ofac").val());
			datos.append('check_laboratorio', $("#checklist_laboratorio").val());
			datos.append('check_global', $("#checklist_global").val());
			datos.append('check_domicilio', $("#checklist_domicilio").val());
			datos.append('check_credito', $("#checklist_credito").val());
			datos.append('check_medico', $("#checklist_medico").val());
			datos.append('check_sex_offender', $("#checklist_sex_offender").val());
			datos.append('check_professional_accreditation', $("#checklist_professional_accreditation").val());
			datos.append('check_ref_academica', $("#checklist_ref_academica").val());
			datos.append('check_nss', $("#checklist_nss").val());
			datos.append('check_ciudadania', $("#checklist_ciudadania").val());
			datos.append('check_mvr', $("#checklist_mvr").val());
			datos.append('check_servicio_militar', $("#checklist_servicio_militar").val());
			//datos.append('check_licencia_manejo', $("#check_licencia_manejo").val());
			datos.append('check_credencial_academica', $("#checklist_credencial_academica").val());
			datos.append('check_ref_profesional', $("#checklist_ref_profesional").val());
			datos.append('id_candidato', $("#idCandidato").val());

			$.ajax({
				url: '<?php echo base_url('Candidato_Conclusion/update_checklist'); ?>',
				type: 'POST',
				data: datos,
				contentType: false,
				cache: false,
				...
				success: function(res){
					...
					if (data.codigo === 1) {
						...
						$("#checklistModal #msj_error").css('display', 'block').html(data.msg);
					}
				}
			});
		}
		function guardarAddress(num){
			...
			$.ajax({
				url: '<?php echo base_url('Client/guardarHistorialDomicilios'); ?>',
				type: 'POST',
				data: datos,
				contentType: false,
				cache: false,
		...
		function guardarComentarioAddress(){
			...
			$.ajax({
				url: '<?php echo base_url('Client/guardarComentarioVerificarDomicilios'); ?>',
				method: "POST",  
		...
		function guardarReferenciaProfesional(num) {
			...
			$.ajax({
				url: '<?php echo base_url('Client/guardarReferenciaProfesional'); ?>',
				type: 'POST',
				data: datos,
				contentType: false,
				cache: false,
		...
		function guardarRefPersonales(cantidad) {
			...
			$.ajax({
				url: '<?php echo base_url('Client/guardarReferenciasPersonales'); ?>',
				type: 'POST',
				data: datos,
				contentType: false,
				cache: false,
		...
		function ejecutarAccion(){
			...
			if(accion == 'generar'){
				$.ajax({
					url: '<?php echo base_url('Client/regenerarPassword'); ?>',
					type: 'post',

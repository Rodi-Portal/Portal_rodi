<div class="container-fluid">
    ...
    <div class="row align-items-center">
      <div class="col-sm-12 col-md-6 col-lg-6">
        <h1 class="titulo_seccion">
          <?php echo t('rec_prog_title', 'Requisiciones en progreso'); ?>
        </h1>
      </div>
      ...
      <div class="col-sm-12 col-md-6 col-lg-6 d-flex justify-content-end">
        ...
        <button type="button" class="btn btn-primary btn-icon-split mr-2" id="btn_nuevo" onclick="openAddApplicant()">
          ...
          <span class="text">
            <?php echo t('rec_prog_btn_register_applicant', 'Registrar aspirante a requisición'); ?>
          </span>
        </button>
        <?php endif; ?>
        ...
        <button type="button" class="btn btn-primary btn-icon-split" data-toggle="modal"
          ...
          <span class="text">
            <?php echo t('rec_prog_btn_change_req_status', 'Cambiar estatus de requisición'); ?>
          </span>
        </button>
        <?php endif; ?>
      </div>
  ...
  <div id="div_requisiciones" class="row">
    <div class="col-6">
      <label>
        <?php echo t('rec_prog_filter_select_req', 'Selecciona una requisición'); ?>
      </label>

      <select class="form-control" name="opcion_requisicion" id="opcion_requisicion">
        <option value="">
          <?php echo t('rec_prog_filter_all', 'Todas'); ?>
        </option>

        <?php if ($reqs): ?>
        <?php foreach ($reqs as $req): ?>
        <option value="<?php echo (int) $req->idReq; ?>">
          <?php
              echo '#' . $req->idReq
              . ' ' . $req->nombre_cliente
              . ' - ' . $req->puesto
              . ' - ' . t('rec_prog_openings_label', 'Vacantes:') . ' ' . $req->numero_vacantes;
          ?>
        </option>
        <?php endforeach; ?>
        <?php endif; ?>
...
<script>
  // Diccionario de traducciones cargadas por CI3
  window.I18N = window.I18N || <?php echo json_encode($this->lang->language, JSON_UNESCAPED_UNICODE); ?>;

  // Translator seguro (con fallback)
  window._t = window._t || function(key, fallback, repl) {
    var s = (window.I18N && window.I18N[key]) ? window.I18N[key] : (fallback || key);

    // Reemplazos opcionales: {name}, {count}, etc.
    if (repl && typeof repl === 'object') {
      for (var k in repl) {
        if (!Object.prototype.hasOwnProperty.call(repl, k)) continue;
        s = s.replace(new RegExp('\\{' + k + '\\}', 'g'), repl[k]);
      }
    }
    return s;
  };

  // Por si en otras partes usas window.t
  window.t = window.t || window._t;
</script>
...
function buildTableProgreso(url) {
  ...
  // Helpers de texto (para no repetir TT() mil veces)
  const T = (k, fb) => TT(k, fb);
  ...
  window.dtTabla = $('#tabla').DataTable({
    ...
    columns: [{
      ...
      {
        title: T('rec_prog_col_applicant', 'Aspirante'),
        data: 'aspirante',
        bSortable: false,
        "width": "15%",
        mRender: function(data) {
      ...
      {
        title: T('rec_prog_col_branch_client', 'Sucursal/Cliente'),
        data: 'nombre_cliente',
        bSortable: false,
        "width": "15%",
      ...
      {
        title: T('rec_prog_col_position', 'Puesto'),
        data: 'puesto',
        bSortable: false,
        "width": "15%",
        mRender: function(data, type, full) {
          return data + '<br>(' + full.numero_vacantes + ' ' + T('rec_prog_openings_label_lower', 'vacantes') +
            ')';
        }
      },
      {
        title: T('rec_prog_col_contact', 'Contacto'),
        data: 'telefono',
        bSortable: false,
        "width": "10%",
        mRender: function(data, type, full) {
          ...
          const correo = norm(full?.correo, T('rec_prog_not_registered', 'No registrado'));
          const medio = norm(full?.medio_contacto, '--');

          return `<b>${T('rec_prog_contact_phone', 'Teléfono')}: </b>${tel}<br>` +
            `<b>${T('rec_prog_contact_email', 'Correo')}: </b>${correo}<br>` +
            `<b>${T('rec_prog_contact_channel', 'Medio')}: </b>${medio}`;
        }
      },
      {
        title: T('rec_prog_col_actions', 'Acciones'),
        data: 'idAsp',
        bSortable: false,
        "width": "10%",
        mRender: function(data, type, full) {
          ...
          var cvLink = allow(P.ASP_CARGAR_DOCS) ?
            ...
            .idAsp + ')" data-toggle="tooltip" title="' + T('rec_prog_tt_upload_docs', 'Cargar documentos') +
            '"><i class="fas fa-upload"></i> ' + T('rec_prog_action_upload_docs', 'Cargar Documentos') +
            '</a>' :
            '';
          ...
          var eliminarAspirante = allow(P.ASP_ELIMINAR_ASP) ?
            ...
            ')" data-toggle="tooltip" title="' + T('rec_prog_tt_delete', 'Eliminar') +
            '"><i class="fas fa-trash"></i> ' + T('rec_prog_action_delete_match', 'Eliminar Match') + '</a>' :
            '';
          ...
          var actualizarDocs = allow(P.ASP_ACTUALIZAR_DOCS) ?
            ...
            full.idAsp + ')" data-toggle="tooltip" title="' + T('rec_prog_tt_update_docs',
              'Actualizar Documentos') + '"><i class="fas fa-eye"></i> ' + T('rec_prog_action_update_docs',
          ...
          var comentarios = allow(P.ASP_COMENTARIOS) ?
            ...
            ', \'' + full.aspirante + '\')"><i class="fas fa-user-tie"></i> ' + T(
              'rec_prog_action_client_comments', 'Comentarios Cliente') + '</a>' :
            '';

          var historial = allow(P.ASP_VER_HIST) ?
            '<a href="javascript:void(0)" id="ver_historial" class="dropdown-item" data-toggle="tooltip" title="' +
            T('rec_prog_tt_view_history', 'Ver historial de movimientos') +
            '"><i class="fas fa-history"></i> ' + T('rec_prog_action_view_history',
              'Ver historial de movimientos') + '</a>' :
            '';

          var iniciar_socio = allow(P.ASP_CAMBIAR_STATUS_REQ) ?
            '<a href="#" id="iniciar_socio" class="dropdown-item" data-toggle="tooltip" title="' + T(
              'rec_prog_tt_send_preemployment',
              'Enviar al módulo de Preempleo para candidatos con o sin estudios previos a la contratación.') +
            '"><i class="fas fa-play-circle"></i> ' + T('rec_prog_action_send_preemployment',
              'Enviar a Preempleo') + '</a>' :
            '';
          ...
          var ingreso = allow(P.ASP_REGISTRAR_MOV) ?
            '<a href="#" id="ingreso_empresa" class="dropdown-item" data-toggle="tooltip" title="' + T(
              'rec_prog_tt_entry_record', 'Registro de datos de ingreso del candidato') +
            '"><i class="fas fa-user-tie"></i> ' + T('rec_prog_action_entry_record', 'Registro de ingreso') +
            '</a>' :
            '';
          ...
          if (allow(P.ASP_EDITAR)) {
            acciones +=
              '<a href="javascript:void(0)" id="editar_aspirante" class="dropdown-item" data-toggle="tooltip" title="' +
              T('rec_prog_tt_edit_applicant', 'Editar aspirante') + '"><i class="fas fa-user-edit"></i> ' + T(
                'rec_prog_action_edit_applicant', 'Editar aspirante') + '</a>';
          }
          if (allow(P.ASP_REGISTRAR_MOV)) {
            acciones +=
              '<a href="javascript:void(0)" id="accion" class="dropdown-item" data-toggle="tooltip" title="' +
              T('rec_prog_tt_register_moves', 'Registrar paso en el proceso del aspirante') +
              '"><i class="fas fa-plus-circle"></i> ' + T('rec_prog_action_register_moves',
                'Registrar movimientos') + '</a>';
          }
          ...
          if (full.status_final == 'FINALIZADO' || full.status_final == 'COMPLETADO') {
            if (full.idCandidato != null && full.idCandidato != '') {
              acciones = '<b>' + T('rec_prog_msg_ese_finished', 'ESE finalizado') + '</b>';
            } else {
              acciones =
                (allow(P.ASP_CAMBIAR_STATUS_REQ) ? iniciar_socio : '') +
                (allow(P.ASP_REGISTRAR_MOV) ? ingreso : '') +
                (allow(P.ASP_EDITAR) ?
                  '<a href="javascript:void(0)" id="editar_aspirante" class="dropdown-item" data-toggle="tooltip" title="' +
                  T('rec_prog_tt_edit_applicant', 'Editar aspirante') + '"><i class="fas fa-user-edit"></i> ' +
                  T('rec_prog_action_edit_applicant', 'Editar aspirante') + '</a>' : '') +
                (allow(P.ASP_REGISTRAR_MOV) ?
                  '<a href="javascript:void(0)" id="accion" class="dropdown-item" data-toggle="tooltip" title="' +
                  T('rec_prog_tt_register_moves', 'Registrar paso en el proceso del aspirante') +
                  '"><i class="fas fa-plus-circle"></i> ' + T('rec_prog_action_register_moves',
                    'Registrar movimientos') + '</a>' : '');
          ...
          return '<div class="btn-group">' +
            ...
            T('rec_prog_actions_btn', 'Acciones') + '</button>' +
            '<div class="dropdown-menu">' + menu + '</div>' +
            '</div>';
        }
      },
      {
        title: T('rec_prog_col_current_status', 'Estatus actual'),
        data: null,
        bSortable: false,
        width: "10%",
        render: function(data, type, row) {
          if (row.status_final !== null && row.status_final !== "") {
            ...
            return T('rec_prog_status_registered', 'Registrado');
          }
        }
      }
    ],
    ...
    rowCallback: function(row, data) {
      ...
      $("a#editar_aspirante", row).bind('click', () => {
        ...
        $('#req_asignada').val(data.id_req).trigger('change');
        $('#nombre').val(nombre[0]);
      ...
      $('a#ver_historial', row).bind('click', () => {
        ...
        $.ajax({
          ...
          success: function(res) {
            ...
            salida += '<th>' + T('rec_prog_hist_date', 'Fecha') + '</th>';
            salida += '<th>' + T('rec_prog_hist_status', 'Estatus') + '</th>';
            salida += '<th>' + T('rec_prog_hist_desc', 'Comentario / Descripción / Fecha y lugar') +
              '</th>';
            salida += '<th>' + T('rec_prog_hist_delete', 'Eliminar Movimiento') + '</th>';
            salida += '</tr>';
            ...
            if (res != 0) {
              ...
              salida += "<tr><td colspan='3' class='text-center'><h5>" + T('rec_prog_hist_none',
                'Sin movimientos') + "</h5></td></tr>";
            }
      ...
      $('a#iniciar_socio', row).off('click').bind('click', () => {
        ...
        $('#puesto').empty()
          .append('<option value="0" selected>' + T('rec_prog_opt_na', 'N/A') + '</option>')
          .append('<option value="otro">' + T('rec_prog_opt_other', 'Otro') + '</option>');
        ...
        var apellidoPaterno = partesNombre.length > 1 ? partesNombre[1] : "";
          ...
          $.ajax({
            ...
            success: function(res) {
              if (res != 0) {
                ...
                $('#puesto').append('<option value="">' + T('rec_prog_opt_select', 'Selecciona') +
                  '</option>');
                ...
                $('#puesto').select2({
                  placeholder: T('rec_prog_select_placeholder', 'Selecciona una opción'),
                  allowClear: true,
                  width: '100%',
                ...
                $('#puesto').append('<option value="">' + T('rec_prog_no_positions',
                  'No hay puestos registrados') + '</option>');
              }
            }
          });
        }, 200);
    ...
    "language": {
      "lengthMenu": T('rec_prog_dt_length', 'Mostrar _MENU_ registros por página'),
      "zeroRecords": T('rec_prog_dt_zero', 'No se encontraron registros'),
      "info": T('rec_prog_dt_info', 'Mostrando registros de _START_ a _END_ de un total de _TOTAL_ registros'),
      "infoEmpty": T('rec_prog_dt_info_empty', 'No hay registros disponibles'),
      "infoFiltered": T('rec_prog_dt_info_filtered', '(Filtrado de _MAX_ registros totales)'),
      "sSearch": T('rec_prog_dt_search', 'Buscar:'),
      "oPaginate": {
        "sLast": T('rec_prog_dt_last', 'Última página'),
        "sFirst": T('rec_prog_dt_first', 'Primera'),
        "sNext": T('rec_prog_dt_next', 'Siguiente'),
        "sPrevious": T('rec_prog_dt_prev', 'Anterior')
      }
    }
  });
...
function mostrarFormularioActualizarDocs(id) {
  ...
  $tbody.html(
    '<tr><td colspan="5" class="text-center">' + T('rec_prog_docs_loading', 'Cargando…') + '</td></tr>'
  );
  ...
  $.ajax({
    ...
    success: function(docs) {
      ...
      if (!Array.isArray(docs) || !docs.length) {
        $tbody.html(
          '<tr><td colspan="5" class="text-center text-muted">' + T('rec_prog_docs_none', 'Sin documentos') +
          '</td></tr>'
        );
        ...
        filas += `
          <tr>
            ...
            <td>
              ...
              <a href="${urlDown}" class="ml-2" title="${T('rec_prog_docs_download', 'Descargar')}" data-toggle="tooltip">
            ...
            <td>
              <button class="btn btn-sm btn-info"
                      data-toggle="tooltip" title="${T('rec_prog_docs_replace_tt', 'Reemplazar archivo')}"
                      onclick="abrirModalReemplazo(${d.id}, '${d.nombre_personalizado.replace(/'/g,'&#39;')}')">
                <i class="fas fa-sync-alt"></i>
              </button>
              <button class="btn btn-sm btn-danger"
                      data-toggle="tooltip" title="${T('rec_prog_docs_delete_tt', 'Eliminar archivo')}"
        ...
        $.ajax({
          ...
          success: function(resp) {
            if (resp && resp.ok) {
              Swal.fire({
                icon: 'success',
                title: T('rec_prog_docs_updated_title', 'Actualizado'),
                text: T('rec_prog_docs_view_updated_ok', 'Vista actualizada correctamente'),
                timer: 1200,
                showConfirmButton: false
              });
            } else {
              Swal.fire(
                T('rec_prog_common_error_title', 'Error'),
                (resp && resp.message) ? resp.message : T('rec_prog_docs_view_update_fail',
          ...
          error: function() {
            Swal.fire(
              T('rec_prog_common_error_title', 'Error'),
              T('rec_prog_docs_view_update_fail', 'No se pudo actualizar la vista.'),
              'error'
            );
          }
        });
    ...
    error: function() {
      Swal.fire({
        icon: 'error',
        title: T('rec_prog_common_error_title', 'Error'),
        text: T('rec_prog_docs_load_fail', 'No se pudieron cargar los documentos')
      });
...
function eliminarAspirante(idAsp) {
  ...
  Swal.fire({
    title: T('rec_prog_swal_confirm_title', '¿Estás seguro?'),
    text: T('rec_prog_swal_confirm_delete_applicant', 'Vas a eliminar este aspirante'),
    icon: 'warning',
    showCancelButton: true,
    confirmButtonText: T('rec_common_yes_delete', 'Sí, eliminar'),
    cancelButtonText: T('rec_common_cancel', 'Cancelar')
  }).then((result) => {
    ...
    if (result.isConfirmed) {
      ...
      // Segunda advertencia
      Swal.fire({
        title: T('rec_prog_swal_warning_title', 'Advertencia'),
        text: T('rec_prog_swal_warning_applicant_hide',
          'El aspirante desaparecerá de este listado y el cliente o sucursal no podrá verlo más'),
        icon: 'error',
        showCancelButton: true,
        confirmButtonText: T('rec_common_accept', 'Aceptar'),
        cancelButtonText: T('rec_common_cancel', 'Cancelar')
      }).then((result2) => {
        ...
        if (result2.isConfirmed) {
          ...
          $.ajax({
            ...
            success: function() {
              ...
              Swal.fire({
                icon: 'success',
                title: T('rec_common_deleted_title', 'Eliminado'),
                text: T('rec_prog_applicant_deleted_ok', 'El aspirante fue eliminado correctamente')
              });
            ...
            error: function() {
              Swal.fire({
                icon: 'error',
                title: T('rec_common_error_title', 'Error'),
                text: T('rec_prog_applicant_delete_fail',
                  'No se pudo eliminar el aspirante. Intenta de nuevo.')
              });
            }
          });
...
function eliminarArchivo(id) {
  ...
  Swal.fire({
    title: T('rec_prog_swal_confirm_title', '¿Estás seguro?'),
    text: T('rec_prog_file_delete_warn', 'Esta acción eliminará el archivo permanentemente.'),
    icon: 'warning',
    showCancelButton: true,
    confirmButtonText: T('rec_common_yes_delete', 'Sí, eliminar'),
    cancelButtonText: T('rec_common_cancel', 'Cancelar')
    ...
    if (result.isConfirmed) {
      ...
      $.ajax({
        url: `<?php echo base_url('Documentos_Aspirantes/eliminar') ?>`,
        type: 'POST',
        data: {
          id
        },
        ...
        success: resp => {
          ...
          if (resp.ok) {
            Swal.fire({
              icon: 'success',
              title: T('rec_common_deleted_title', 'Eliminado'),
              text: (resp && resp.message) ? resp.message : T('rec_prog_file_deleted_ok',
            ...
            Swal.fire(
              T('rec_common_error_title', 'Error'),
              (resp && resp.message) ? resp.message : T('rec_prog_file_delete_fail',
        ...
        error: () => {
          Swal.fire(
            T('rec_common_error_title', 'Error'),
            T('rec_prog_file_delete_fail', 'No se pudo eliminar el archivo.'),
            'error'
          );
        }
      });
  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      try {
        ...
        Swal.fire({
          icon: 'error',
          title: TT('rec_common_server_error_title', 'Error en el servidor'),
          html: "<b>" + TT('rec_common_server_unexpected_reply', 'Respuesta inesperada del servidor:') +
            "</b><br><pre style='text-align:left'>" + res + "</pre>",
          width: '50em'
        });
...
function trUsuarioRol(valor) {
  const v = (valor ?? '').toString().trim();
  const k = v.toLowerCase();

  if (k === 'cliente' || k === 'client') {
    return _t('rec_common_role_client', 'Cliente');
  }

  if (k === 'reclutador' || k === 'recruiter') {
    return _t('rec_common_role_recruiter', 'Reclutador');
  }

  // Si es nombre real u otro texto, no lo toques
  return v || '--';
}
function getIngresoCandidato(id) {
  $.ajax({
    ...
    success: function(res) {
      ...
      var salida = '<table class="table table-striped" style="font-size: 14px">';
      salida += '<tr style="background: gray;color:white;">';
      salida += '<th>' + _t('rec_prog_warranty_th_date', 'Fecha registro') + '</th>';
      salida += '<th>' + _t('rec_prog_warranty_th_user', 'Usuario') + '</th>';
      salida += '<th>' + _t('rec_prog_warranty_th_desc_status', 'Descripción / Estatus') + '</th>';
      salida += '</tr>';
      ...
      if (res != 0) {
        ...
        for (var i = 0; i < dato.length; i++) {
          ...
          salida += "<tr>";
          salida += '<td>' + fecha + '</td>';
          salida += '<td>' + trUsuarioRol(dato[i]['usuario']) + '</td>';
          salida += '<td>' + dato[i]['descripcion'] + '</td>';
          salida += "</tr>";
        }
      } else {
        salida += "<tr>";
        salida += '<td colspan="4" class="text-center"><h5>' + _t('rec_prog_warranty_no_records', 'Sin registros') + '</h5></td>';
        salida += "</tr>";
      }
...
function verHistorialBolsaTrabajo(id, nombreCompleto) {
  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      var salida = '<table class="table table-striped" style="font-size: 14px">';
      salida += '<tr style="background: gray;color:white;">';
      salida += '<th>' + TT('rec_prog_hist_tbl_date', 'Fecha') + '</th>';
      salida += '<th>' + TT('rec_prog_hist_tbl_user', 'Usuario') + '</th>';
      salida += '<th>' + TT('rec_prog_hist_tbl_comment', 'Comentario / Estatus') + '</th>';
      salida += '</tr>';
      ...
      if (res != 0) {
        ...
        salida += "<tr>";
        salida += '<td colspan="3" class="text-center"><h5>' + TT('rec_prog_hist_no_comments', 'Sin comentarios') + '</h5></td>';
        salida += "</tr>";
      }
...
function eliminarRegistro(id) {
  Swal.fire({
    title: _t('rec_prog_swal_delete_confirm_title', '¿Estás seguro?'),
    text: _t('rec_prog_swal_delete_confirm_text', '¡Esta acción no se puede deshacer!'),
    icon: 'warning',
    showCancelButton: true,
    confirmButtonText: _t('rec_prog_swal_delete_confirm_btn_yes_continue', 'Sí, continuar'),
    cancelButtonText: _t('rec_prog_swal_btn_cancel', 'Cancelar')
  }).then((result) => {
    if (result.isConfirmed) {
      // Segunda confirmación
      Swal.fire({
        title: _t('rec_prog_swal_delete_confirm2_title', '¿Realmente deseas eliminar el registro?'),
        text: _t('rec_prog_swal_delete_confirm2_text', 'Esta acción es definitiva.'),
        icon: 'question',
        showCancelButton: true,
        confirmButtonText: _t('rec_prog_swal_delete_confirm2_btn_yes_delete', 'Sí, eliminar'),
        cancelButtonText: _t('rec_prog_swal_btn_cancel', 'Cancelar')
      }).then((result2) => {
        if (result2.isConfirmed) {
          ...
          $.ajax({
            ...
            success: function(response) {
              try {
                ...
                if (res.status) {
                  Swal.fire(
                    _t('rec_prog_swal_deleted_title', 'Eliminado'),
                    res.mensaje,
                    'success'
                  );
                  ...
                  Swal.fire(
                    _t('rec_prog_swal_attention_title', 'Atención'),
                    res.mensaje,
                    'warning'
                  );
                }
                ...
                Swal.fire(
                  _t('rec_prog_swal_error_title', 'Error'),
                  _t('rec_prog_swal_unexpected_server_response', 'Respuesta inesperada del servidor.'),
                  'error'
                );
            ...
            error: function(xhr, status, error) {
              Swal.fire(
                _t('rec_prog_swal_error_title', 'Error'),
                _t('rec_prog_swal_delete_ajax_error', 'Ocurrió un error al eliminar.'),
                'error'
              );

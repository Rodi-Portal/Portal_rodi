...
<body>
        ...
        <div class="modal-body">
          ...
          <form id="nuevoRegistroForm">
            ...
            <div class="row">
              ...
              if($id_cliente != 172 && $id_cliente != 178 && $id_cliente != 205){ ?>
                <div class="col-4">
                  <label>Puesto *</label>
  ...
  <div class="modal fade" id="avancesModal" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Mensajes de progreso: <br><span class="nombreCandidato"></span></h4>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light" id="menu">
      ...
      <div class="collapse navbar-collapse" id="navbarNavDropdown">
        <ul class="navbar-nav ml-auto">
          <?php 
					//* Se permite el registro a todos los clientes comentando esta linea
          //if($this->session->userdata('idcliente') == 16 || $this->session->userdata('idcliente') == 159 || $this->session->userdata('idcliente') == 172 || $this->session->userdata('idcliente') == 178 || $this->session->userdata('idcliente') == 205 || $this->session->userdata('idcliente') == 242){ ?>
            <li class="nav-item">
        ...
        <div class="contenedor mt-5 my-5">
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h4 class="mb-2 font-weight-bold text-primary  text-center">Lista de candidatos</h4>
            </div>
            <div class="card-body">
  ...
  <script>
	...
	<script>
      ...
      var beca_url = '<?php echo base_url(); ?>_beca/';
			...
			changeDatatable(url, psico, beca_url);
      ...
      if(id_cliente == 159){
		...
		function changeDatatable(url, psico, beca_url) {
			$('#tabla').DataTable({
				"pageLength": 25,
				//"pagingType": "simple",
				"order": [0, "desc"],
              ...
              if(full.tipo_conclusion != 0){
                ...
                if (full.status_bgc == 0) {
                  if(full.fecha_nacimiento != null && full.fecha_nacimiento != '0000-00-00'){
                    ...
                    return '<i class="fas fa-circle status_bgc0"></i> Por iniciar';
                  }
                } else {
                  if (full.liberado == 1){
                    ...
                    if(full.status_bgc > 0){
                      ...
                      return '<i class="fas fa-circle status_bgc0"></i> Por finalizar';
                    }
                  }
                  else{
                    return '<i class="fas fa-circle status_bgc0"></i> Por finalizar';
                ...
                if(full.id_cliente == 33){
                  if (full.idBeca != null && full.archivoBeca != '') {
                    return '<a href="' + beca_url + full.archivoBeca + '" data-toggle="tooltip" title="Descargar solicitud de beca" class="fa-tooltip icono_datatable" download><i class="fas fa-file-excel"></i></a>';
                  } else {
                    return 'Pendiente';
                  }
                }

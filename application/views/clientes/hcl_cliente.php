	...
	<header>
		<nav class="navbar navbar-expand-lg navbar-light bg-light" id="menu">
			...
			<div class="collapse navbar-collapse" id="navbarNavDropdown">
				...
				<?php 
				if(empty($bloqueo)){ 
					$espectador = $this->session->userdata('espectador');
				 if ($espectador != 1): ?>
					<li class="nav-item">
  ...
  <script>
	...
	<script>
      ...
      mensajeBloqueado = '<?php echo $mensaje; ?>';
			if (estaBloqueado == 1) {
				$('#bloqueoModal #titulo_modal').html('IMPORTANT NOTICE <i class="fas fa-exclamation-triangle"></i>');
		...
		$(document).ready(function() {
  		...
  		$('.div_info_project, .div_project, .div_info_previo, .div_previo, .div_info_check, .div_check, .div_info_test, .div_test, .div_info_extra, .div_extra').css('display','none');
			...
			var url = '<?php echo base_url('Client/getCandidatoPanelCliente'); ?>';
			var espectador = '<?php echo $espectador ?>' ;
...
//Name					
					{
						...
						mRender: function(data, type, full){
							return '<span data-toggle="tooltip" title="'+full.id+'">'+data+'</span>';
						}
					},
//Proyect					
					{
...
//columna BGV
					{
										...
										bSortable: false,
										"width": "10%",
										// Oculta la columna si espectador es 1
										render: function(data, type, full) {

											let pdfPrevio = '';
											if (id_cliente == 233 && full.idBGC != null) {
												pdfPrevio = '<div style="display: inline-block;"><form id="formBGV' + data + '" action="<?php echo base_url('Client/crearReportePDF'); ?>" method="POST"><a href="javascript:void(0);" data-toggle="tooltip" title="Descargar documento final" id="pdfFinal" class="fa-tooltip icono_datatable"><i class="fas fa-file-powerpoint"></i></a><input type="hidden" name="idCandidatoPDF" id="idCandidatoPDF' + data + '" value="' + data + '"></form></div>';
											}

											if (full.status == 0) {
												return "Pending" + pdfPrevio;
											}

											if ((full.status == 1 || full.status == 2) && full.status_bgc == 0) {
												return "In process" + pdfPrevio;
											}
											...
											if (full.status_bgc != 0) {
												if (full.liberado == 1) {
													...
													var pdf = '<div style="display: inline-block;"><form id="formBGV' + data + '" action="<?php echo base_url('Client/crearReportePDF'); ?>" method="POST"><a href="javascript:void(0);" data-toggle="tooltip" title="Descargar documento final" id="pdfFinal" class="fa-tooltip icono_datatable"><i class="fas fa-file-pdf"></i></a><input type="hidden" name="idCandidatoPDF" id="idCandidatoPDF' + data + '" value="' + data + '"></form></div>';
													return icono + pdf;
												}
												else{
													return "In process" + pdfPrevio;
